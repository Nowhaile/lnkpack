...
int main(int argc, const char **argv)
{
    ...
    if (argc < 2)
    {
        ...
        printf("Usage: lnkpack <output_lnk> <input_ext> [-d <input_description>] [-i <input_ico>] [-s]\n\n");
        printf("Example: \n");
        printf("lnkpack \"Hello App\" assets\\HelloApp.exe -d \"Hello App Link\" -i assets\\icon.ico -s\n");
        printf("Flag -s silents output");
        printf("\n");
    ...
    {
        char *user_home = malloc(MAX_PATH + 1);
        if (get_user_path(user_home))
        {
            location = strreplace(location, "{UserDesktop}", user_home, true);
            if(location == NULL){
                sendMessage("warn", "Filed replacing {UserDesktop} makro", NULL);
                return 1;
            }
        }
        else
        {
            sendMessage("warn", "Can`t find user home dir", NULL);
            return 1;
        }
    }
    ...
    if (!silent)
        sendMessage("info", "Creating lnk for ", str_prbrk(argv[2], "/\\", false));
